...
const Editor = dynamic(() => import('@monaco-editor/react'), {
  ssr: false,
  loading: () => (
    <div className="flex items-center justify-center h-full bg-gray-50 text-gray-600">에디터 로딩중...</div>
  ),
});
...
export function CodeEditor({ onMint, disabled, lesson, chapter }: CodeEditorProps) {
  ...
  const getProcessedCode = useCallback(() => {
    return LESSON[lesson][chapter].code
      ...
      <CardContent>
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">기본 속도 (m / h)</label>
              <input
                type="number"
                min="10"
                max="1000"
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
              <p className="text-xs text-gray-500 mt-1">코드 예시에서 자동 전진 속도를 조정합니다.</p>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">참치 보너스 (m)</label>
              <input
                type="number"
                min="1"
                max="500"
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
              <p className="text-xs text-gray-500 mt-1">참치를 먹였을 때 추가 이동할 거리를 설정합니다.</p>
            </div>
          </div>
        </div>
      </CardContent>
