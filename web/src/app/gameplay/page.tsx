...
function GameplayContent() {
  ...
  const handleMintSwimmer = async (packageId: string) => {
    if (!currentAccount) {
      alert('Connect your wallet first.');
      return;
    }
    ...
    setActionLoading('mintSwimmer');
    try {
      const tx = new Transaction();
      tx.moveCall({
        target: `${packageId}::swimming::mint_swimmer`,
        arguments: [],
      });
  ...
  const handleMintTuna = async (packageId: string) => {
    if (!currentAccount) {
      alert('Connect your wallet first.');
      return;
    }
    ...
    try {
      const tx = new Transaction();
      tx.moveCall({
        target: `${packageId}::swimming::mint_tuna`,
        arguments: [],
      });
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <div className={''}>
          <SwimmingPool
            ...
            onMintSwimmer={handleMintSwimmer}
