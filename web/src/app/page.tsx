...
export default function Home() {
  ...
  const sidebarHeader = (
    <div>
      <p className="text-xs uppercase tracking-wide text-blue-600 font-semibold">Sui-mmers 코스 가이드</p>
      <h2 className="mt-1 text-base font-bold text-gray-900">Session별 학습 일정</h2>
    </div>
  )

  const sidebarOverlayDescription = '세션 흐름을 따라 Move & Sui 학습을 진행해보세요.'
  ...
  return (
    <LearningLayout
        ...
        <header className="bg-white border-b border-gray-200">
          <div className="container mx-auto px-4 py-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-3">
              <Button variant="outline" className="lg:hidden" onClick={() => setIsCourseOpen(true)}>
                📘 코스 열람
              </Button>
              <WalletConnect />
            </div>
      ...
      sidebar={<CourseOutline />}
      overlayTitle="Sui-mmers 코스 가이드"
      overlayDescription={sidebarOverlayDescription}
      isSidebarOpen={isCourseOpen}
      onSidebarClose={() => setIsCourseOpen(false)}
      footer={
